...
function jobqueue_install {
    docker exec -it pbs-master /bin/bash -c "cd /jobqueue_features; mkdir dask-worker-space; chmod 777 dask-worker-space"
    docker exec -it pbs-master /bin/bash -c "cd /jobqueue_features; pip install -r requirements.txt; pip install --no-deps -e ."
    docker exec -it pbs-slave-1 /bin/bash -c "cd /jobqueue_features; mkdir dask-worker-space; chmod 777 dask-worker-space"
    docker exec -it pbs-slave-1 /bin/bash -c "cd /jobqueue_features; pip install -r requirements.txt; pip install --no-deps -e ."
    docker exec -it pbs-slave-2 /bin/bash -c "cd /jobqueue_features; mkdir dask-worker-space; chmod 777 dask-worker-space"
