...
class TestClusters(TestCase):
    ...
    def test_nonexistent_queue_type(self):
        ...
        self.assertIn("queue_type kwarg value 'chicken' not in available options", context.exception.message)
    ...
    def test_mpi_job_cluster(self):
        ...
        self.assertIsInstance(cluster.client, Client)
        with self.assertRaises(ValueError):
            get_cluster(queue_type='knl', mpi_mode=True, nodes=64,
                        cores=64, **self.kwargs)
    ...
    def test_mpi_multi_node_job_cluster(self):
        ...
        with self.assertRaises(ValueError):
            get_cluster(queue_type='knl', cpus_per_task=37, cores=2,
                        mpi_mode=True, **self.kwargs)
        with self.assertRaises(ValueError):
            get_cluster(queue_type='knl', cores=1,
                        ntasks_per_node=13, mpi_mode=True, **self.kwargs)

    def test_mpi_complex_job_cluster(self):
        # Now a few more variables
        cluster = get_cluster(queue_type='gpus', mpi_mode=True, nodes=2, ntasks_per_node=4, **self.kwargs)
        ...
        self.assertIn('export OMP_NUM_THREADS=${SLURM_CPUS_PER_TASK}', cluster.job_script())
    ...
    def test_mpi_complex_job_cluster_fail(self):
        ...
        with self.assertRaises(ValueError):
            ...
            cluster = get_cluster(queue_type='gpus', mpi_mode=True, cores=2, ntasks_per_node=2, **self.kwargs)
        # If you really want this you ask for it explicitly
        cluster = get_cluster(queue_type='gpus', mpi_mode=True, cores=2, ntasks_per_node=2, cpus_per_task=1,
                              **self.kwargs)
        self.assertIn('#SBATCH --cpus-per-task=1', cluster.job_header)
        ...
        cluster = get_cluster(queue_type='gpus', mpi_mode=True, nodes=1, ntasks_per_node=2, **self.kwargs)
    ...
    def test_mpi_explicit_job_cluster(self):
        # Now a few more variables
        cluster = get_cluster(queue_type='gpus', mpi_mode=True, nodes=2, cpus_per_task=2, ntasks_per_node=12,
                              **self.kwargs)
    ...
    def test_wrong_hyperthreading_factor(self):
        with self.assertRaises(ValueError):
            get_cluster(SLURM, minimum_cores=2, cores_per_node=1,
                        hyperthreading_factor=1, **self.kwargs)
