...
def get_features_kwarg(name, scheduler=None, queue_type=None, default=None):
    ...
    if scheduler is not None and queue_type is not None:
        value = config.get('jobqueue-features.{}.queue-type.{}.{}'.format(
            scheduler, queue_type, name), default=None)
...
class CustomClusterMixin(object):
    ...
    def get_kwarg(self, name, default=None):
        return get_features_kwarg(name=name,
    ...
    def _get_queue_type(self, queue_type, default=None):  # type: (str) -> None
        ...
        if queue_type:
            ...
            try:
                avail_queue_types = [
                    key for key in config.config['jobqueue-features'][self.scheduler_name]['queue-type']
                ]
            except KeyError:
                avail_queue_types = []
            ...
            if queue_type not in avail_queue_types:
                raise ValueError("queue_type kwarg value '{queue_type}' not in available options: "
                                 "{av_queue_types}".format(queue_type=queue_type, av_queue_types=avail_queue_types))
    ...
    def _get_cores_per_node(self, cores_per_node, default=1):
        # type: (int) -> None
        self.cores_per_node = cores_per_node if cores_per_node is not None \
    ...
    def _get_hyperthreading_factor(self, hyperthreading_factor, default=1):
        ...
        self.hyperthreading_factor = \
            hyperthreading_factor if hyperthreading_factor is not None \
    ...
    def _get_minimum_cores(self, minimum_cores, default=1):
        # type: (int) -> None
        self.minimum_cores = minimum_cores if minimum_cores is not None \
    ...
    def _get_mpi_mode(self, mpi_mode,  default=False):  # type: (bool) -> None
        self.mpi_mode = mpi_mode if isinstance(mpi_mode, bool) else \
            self.get_kwarg(name='mpi-mode', default=default)

    def _get_mpi_launcher(self, mpi_launcher, default=None):  # type: (str) -> None
        self.mpi_launcher = mpi_launcher or self.get_kwarg(name='mpi-launcher',
                                                           default=default)
        if self.mpi_mode and not self.mpi_launcher:
            raise ValueError("When using MPI mode, an MPI launcher (such as srun, mpirun,...) must be set via the"
    ...
    def _update_kwargs_cores(self, **kwargs):  # type: (Dict[...]) -> Dict[...]
        ...
        if self.mpi_mode:
            ...
            self.ntasks_per_node = kwargs.get('ntasks_per_node',
                                              self.get_kwarg('ntasks-per-node'))
            self.cpus_per_task = kwargs.get('cpus_per_task', self.get_kwarg('cpus-per-task'))
            cores = kwargs.get('cores')
            if cores:
                expected_cores = cores
                self.warnings.append("In MPI mode we assume that when you provide 'cores' you mean total number of "
            ...
            if self.nodes is None:
                ...
                if expected_cores is None:
                    raise ValueError('Not enough information to derive MPI/OpenMP configuration')
                if self.ntasks_per_node is None:
                    ...
                    if self.cpus_per_task is None:
                        if self.cores_per_node % self.ntasks_per_node != 0:  # assume physical
                            raise ValueError('"cores_per_node" not divisible by "ntasks_per_node"')
                        else:
                            self.cpus_per_task = self.cores_per_node / self.ntasks_per_node
                ...
                if self.cpus_per_task is None:
                    if self.ntasks_per_node is None:
                        ...
                        if self.ntasks_per_node > self.cores_per_node * self.hyperthreading_factor:
                            raise ValueError('"ntasks_per_node" cannot be higher than ' +
                                             '"cores_per_node" * "self.hyperthreading_factor"')
                        if self.ntasks_per_node < self.cores_per_node:  # assume physical is what we want
                            self.cpus_per_task = self.cores_per_node / self.ntasks_per_node  # safe since ints
                        else:
                            self.cpus_per_task = 1
                else:
                    if self.ntasks_per_node is None:
                        self.ntasks_per_node = 1
                    if self.ntasks_per_node * self.cpus_per_task > self.cores_per_node * self.hyperthreading_factor:
                        raise ValueError('"ntasks_per_node" * "cpus_per_task" cannot be higher than ' +
                                         '"cores_per_node" * "self.hyperthreading_factor"')
            ...
            if any(k in kwargs for k in
                   ('mpi_launcher', 'nodes', 'ntasks_per_node', 'cpus_per_task',
                    'openmp_env_extra')):
                self.warnings.append('kwargs mpi_launcher, nodes, ntasks_per_node, '
            ...
            if features_cores > self.cores_per_node * self.hyperthreading_factor:
                raise ValueError("cores cannot be > {} (cores_per_node * hyperthreading_factor)".format(
                                 self.cores_per_node * self.hyperthreading_factor))
    ...
    def _update_kwargs_job_extra(self, **kwargs):  # type: (Dict[...]) -> Dict[...]
        ...
        if job_extra is None:
            job_extra = config.get('jobqueue.{}.job_extra'.format(self.scheduler_name), default=[])
    ...
    def _update_kwargs_env_extra(self, **kwargs):  # type: (Dict[...]) -> Dict[...]
        ...
        if not env_extra:
            env_extra = config.get('jobqueue.{}.env_extra'.format(self.scheduler_name), default=[])
